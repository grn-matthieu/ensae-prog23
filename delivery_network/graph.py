...
def find_LCA(input_graph, node1, node2):
    ancestors = set()
    current_node = dict_of_nodes[node1]
    lca = dict_of_nodes[node2]
    while current_node.rank != 0:
        ancestors.add(current_node.parent)
        current_node = current_node.parent
    
    #We have created a list of ancestors for the first node
    #Now we search the LCA by testing is each parent of node2 is in ancestors
